...
<head>
    ...
    <script>
        ...
        asset_new_type.initialize = () => {
            let parent = asset_new_type.loader.host.parentElement;
            parent.children[0].children[0].textContent = "Add new asset type";
        };
        asset_new_type.paramsPassed = (fromView, params) => {
            //console.log(params);
        };
        asset_new_type.finalize = () => {
            ViewloaderInstances[0].sendParams(asset_new_type.loader, asset_new_type.types.map(t => t.name).length);
        };
    </script>
</head>
...
<body>
    ...
    <div style="width:80%;margin:0 auto; margin-top:2em">
        <b style="text-align:left;margin-left: 10px;">Insert the name of new Asset type</b>
        <div class="input-form-v1" title="Parent type used to import/inhirate fields from">
            <label for="asset_name">Parent type</label>
            <select id="parent_select" class="input" onchange="parentTypeChanged(this)">
        ...
        <div class="input-form-v1">
            ...
            <input type="text" id="asset_name" name="asset_name" form="f1">
        </div>
        <div class="input-form-v1" title="Shared asset can be assigned/handed to users from various sections accross the organaization. set it to specific section will make only visible to users from the same section">
            <label for="asset_section">Asset Section</label>
            <select class="input" id="asset_section" name="asset_section" form="f1">
        ...
        <div class="input-form-v1" title="Unique assets can't have duplicates(quantity is 1)">
            <label for="asset_is_unique">Unique asset type</label>
            <select class="input" id="asset_is_unique" name="asset_is_unique" form="f1">
                <option value="0" selected>No</option>
                <option value="1">Yes</option>
            </select>
        </div>
        <div class="input-form-v1" title="Multi owner assets can be assigned to multiple users">
            <label for="asset_multiowner">Allow multiple owners</label>
            <select class="input" id="asset_multiowner" name="asset_multiowner" form="f1">
                <option value="0" selected>No</option>
                <option value="1">Yes</option>
            </select>
        </div>
            ...
            <div relative w95>
                ...
                <div class="input-form-v1">
                    ...
                    <select class="input" form="f1" id="type_value__from" onchange="fieldTypeChanged(this)">
                        ...
                        <option value="list-users">User</option>
                        <option value="list-values">Set of values</option>
                ...
                <div class="input-form-v1" id="max_length_value_div" hidden>
                    ...
                    <input type="number" id="max_length__form" name="max_length__form" form="f1" step="1" minlength="1" placeholder="at least 1">
                </div>
                ...
                <div class="input-form-v1" id="min_length_value_div" hidden>
                    ...
                    <input form="f1" type="number" id="min_length__form" name="min_length__form" form="f1" step="1" minlength="1" placeholder="at least 1">
                </div>
                ...
                <div class="input-form-v1" id="max_value_value_div" hidden>
                    ...
                    <input form="f1" type="number" id="max_value__form" name="max_value__form" form="f1" step="1" min="1" placeholder="at least 1">
                </div>
                <!-- Field to set the min length if type is text -->
                <div class="input-form-v1" id="min_value_value_div" hidden>
                    <label for="min_value__form">Min value</label>
                    <input form="f1" type="number" id="min_value__form" name="min_value__form" form="f1" step="1" min="1" placeholder="at least 1">
                </div>
    ...
    <script>
        fieldCollection.setContainer("fields_container");

        async function parentTypeChanged(self) {
            const types = asset_new_type.types;
            const value = self.selectedOptions[0].value;
            let confirmed = await confirmAlert("question", "Are you sure?", "Changing the value will cause to remove all created fields. Proceed ?")
            if (confirmed) {
        ...
        function insertFields(type = {}) {
            JSON.parse(type.fields).forEach(field => {
               let values =  "";
               if(field.values) {
                ...
                const object = {
                    ...
                    field_targetvalues: values,
                    field_default: field?.default ?? "",
                    field_maxlength: field?.maxlength ?? 100000,
                    field_minlength: field?.minlength ?? 0,
                    field_maxvalue: field?.maxvalue ?? 9999999999,
                    field_minvalue: field?.minvalue ?? 9999999999,
                    field_multiselect: field?.multiselect ?? "Invalid",
                    field_booleandefault: field?.default ?? "",
                }
                fieldCollection.processFieldData2(object); /* store and disply the imported fields */
            });
        }
        ...
        function fielMultiChooseChanged(self = HTMLSelectElement) {
            ...
            if (typeValue == "list-users" ||
                typeValue == "file" || typeValue == "video" || typeValue == "image") return;
            if (value == "1") {
        ...
        function fieldTypeChanged(self = HTMLSelectElement) {
            ...
            default_value__form.type = "text";

            required_value__from.options.item(0).selected = true;
            ...
            if (value == "list-users") {
                ...
                .forEach(e => e.setAttribute("hidden", ""));
            } else if (value == "file" || value == "video" || value == "image") {
                [multi_select_div, required_value_div].forEach(e => e.removeAttribute("hidden"));

                [min_length_value_div, max_length_value_div,
                    target_values_div, max_value_value_div,
                    min_value_value_div, default_value_div
                ]
                .forEach(e => e.setAttribute("hidden", ""));
            } else if (value == "datetime") {
                default_value__form.type = "date";
                [required_value_div, default_value_div].forEach(e => e.removeAttribute("hidden"));

                [multi_select_div, min_length_value_div, max_length_value_div,
                    target_values_div, max_value_value_div,
                    min_value_value_div, default_value_div
                ]
                .forEach(e => e.setAttribute("hidden", ""));
        ...
        function requiredValueCharnged(self) {
            ...
            if (typeValue == "file" || typeValue == "video" || typeValue == "image") {
                [boolean_default_value_div, default_value_div].forEach(e => e.setAttribute("hidden", ""));
                return;
            }
            if (value == "1") {
